...
\name{statistic.sim}
\alias{statistic.sim}
\title{Simulates statistics related to exceedance region.}
\usage{
statistic.sim(krige.obj, level, alternative = "less", ...)
}
\arguments{
\item{krige.obj}{An object from the function \code{krige.uk} in the \code{SpatialTools} package.}

\item{level}{The threshold/exceedance level under consideration.}

\item{alternative}{Indicates the type of exceedance region or level curve under consideration.  For exceedances above a threshold, use (\code{alternative = "less"}).  For exceedances below a threshold, use (\code{alternative = "greater"}).  For contour lines, use (\code{alternative = "two.sided"}). Defaults to "less".}

\item{...}{Additional arguments when \code{alternative = "two.sided"}. See Details.}
}
\value{
